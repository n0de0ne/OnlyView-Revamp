...
| **Golden hour** — a living photograph: the villa's own shot of the bay from the pool, animated region by region in WebGL (three.js / react-three-fiber): the sea swells and glitters, the pool ripples where you touch it, clouds and trees move, and the hour sinks the sun, turns the sky violet then navy, brings out the stars and lights the pool. The region masks are derived from the photograph itself by `scripts/golden-mask.mjs` at full resolution: a rough pass (edge detection for the hills, colour for the foliage, fitted lines for the pool, the umbrella measured by hand), then a colour guided filter with the photo as guide that snaps every contour to the real edges at sub-pixel precision. Two lossless WebPs, no alpha channel | `/`, `/location` |
